...
(() => {
  ...
  term.innerHTML = '<div class="cursor-line"><span class="g">root@catasic</span>:~$ <span class="cursor"></span></div>';
  let bestLine = null;
  ...
  if (ready) {
    const links = [
      ...
      cfg.token && [`https://app.uniswap.org/swap?chain=base&outputCurrency=${cfg.token}`, "buy.ctasic"],
      [`https://opensea.io/assets/base/${cfg.nft}/1`, "opensea"],
    ].filter(Boolean);
    $("startList").innerHTML = links.map(([h, l]) => `<a href="${h}" target="_blank" rel="noopener">./${l}</a>`).join("");
  }

  // clock (UTC, like a server log)
  const tick = () => { $("clock").textContent = new Date().toISOString().slice(11, 19) + " UTC"; };
  tick(); setInterval(tick, 1000);

  // hash stream: real hashes from the CPU workers scroll past
  const stream = $("stream");
  function pushHash(h) {
    if (!stream) return;
    ...
    d.textContent = h;
    stream.prepend(d);
    while (stream.children.length > 14) stream.lastChild.remove();
  }
  ...
  // ============================================================ boot
  function glitch() {
    const t = document.querySelector(".logo");
    if (!t || reduced) return;
    t.classList.remove("glitching"); void t.offsetWidth; t.classList.add("glitching");
  }
  async function boot() {
    ...
    let skip = false;
    const finish = () => { el.classList.add("done"); setTimeout(() => el.remove(), 700); glitch(); };
    ...
    const line = (text, cls) => { const d = document.createElement("div"); if (cls) d.className = cls; d.textContent = text; txt.appendChild(d); return d; };
    const steps = [
      ["handshake base.mainnet chain=" + cfg.chainId, ready ? "OK" : "NO CONTRACTS"],
      ["probe cpu // " + cores + " threads", "OK"],
      ["probe gpu // webgpu", gpuOk ? "OK" : "CPU MODE"],
      ["load sprites // 11 layers 124 traits", "OK"],
      ["load kernel // keccak256 x" + (gpuOk ? "gpu+cpu" : "cpu"), "OK"],
    ];
    line("$ ./catasic --connect", "hi");
    for (const [label, res] of steps) {
      if (skip) return;
      const d = line("");
      for (let p = 0; p <= 20; p += 4) {
        if (skip) return;
        d.textContent = `> ${label.padEnd(42, " ")} [${"#".repeat(p)}${".".repeat(20 - p)}] ${String(p * 5).padStart(3)}%`;
        await wait(35);
      }
      d.textContent += "  " + res;
      if (res !== "OK") d.className = "warn";
    }
    if (skip) return;
    line("");
    line("ACCESS GRANTED", "granted");
    await wait(650);
    if (!skip) finish();
  }
  ...
  function drawProgress(minted) {
    const box = $("prog");
    const cells = Math.max(16, Math.floor(box.clientWidth / 11));
    const on = Math.round(cells * minted / WALL);
    box.textContent = "█".repeat(on) + "░".repeat(Math.max(0, cells - on));
  }
  ...
  window.addEventListener("resize", () => { if (state) drawProgress(Number(state[1])); });
  epochRows();
  boot();
